...
| `--subject`=STR       | Attribute holding user identity in User info                |
...
| `--org`=STR           | IdP-specific organization (tennant or realm) ID             |
...
| `--idp-user`=STR      | External IdP's user name (subject)                          |
...
the help of these templates, user's email can be set to `--idp-user` to match
