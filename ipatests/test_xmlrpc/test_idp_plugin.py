...
class TestCreateIdpClientAuth(XMLRPC_test):
    """Tests for client authentication with certificate (PR #8308)."""
    ...
    def test_switch_to_client_secret(self, method, pkcs12_path):
        ...
        with raises_exact(errors.MutuallyExclusiveError(
            ...
            command()
        idp.update(updates=dict(ipaidpclientauthmethod='client_secret'))
        command = idp.make_update_command(
    ...
    def test_switch_cert_auth(self, pkcs12_path):
        ...
        idp = IdpTracker(
            ...
            ipaidpclientsecret=PKCS12PASSWORD)
        idp.create()
        idp.update(updates=dict(ipaidpclientauthmethod='private_key_jwt'))
        idp.update(updates=dict(ipaidpclientauthmethod='tls_client_auth'))
        idp.delete()
